    ...
    with sync_engine.connect() as conn:
        ...
        if "role" in columns:
            ...
            for name, r_id in role_map.items():
                ...
                stmt = text(f"UPDATE users SET role_id = :rid WHERE CAST(role AS TEXT) = :name")
            ...
            # conn.commit()
            logger.warning("Skipping DROP COLUMN role for safety. Please verify data and drop manually or update code references first.")
