...
class ConnectionManager:
    ...
    async def connect_positions(self, websocket: WebSocket):
        ...
        logger.info(
            "websocket_connected",
            channel="positions",
            total=len(self.position_connections),
        )
    ...
    async def connect_gates(self, websocket: WebSocket):
        ...
        logger.info(
            "websocket_connected", channel="gates", total=len(self.gate_connections)
        )
    ...
    async def connect_alerts(self, websocket: WebSocket):
        ...
        logger.info(
            "websocket_connected", channel="alerts", total=len(self.alert_connections)
        )
    ...
    def disconnect_gates(self, websocket: WebSocket):
        ...
        logger.info(
            "websocket_disconnected", channel="gates", total=len(self.gate_connections)
        )
    ...
    def disconnect_alerts(self, websocket: WebSocket):
        ...
        logger.info(
            "websocket_disconnected",
            channel="alerts",
            total=len(self.alert_connections),
        )
...
def serialize_gate(gate: Gate) -> dict:
    ...
    return {
        ...
        "lastStateChange": (
            gate.last_state_change.isoformat() if gate.last_state_change else None
        ),
...
async def websocket_positions(
    ...
    try:
        ...
        async for db in get_db():
            query = (
                select(RTLSPosition).order_by(RTLSPosition.timestamp.desc()).limit(100)
            )
            if floor:
