...
function CameraThumbnail({ camera, refreshInterval, onClick }: CameraThumbnailProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-2">
        ...
        <p className="text-xs text-slate-400 truncate">{camera.zone || camera.floor}</p>
      </div>

      {/* Status indicator */}
      <div className={`absolute top-2 right-2 w-2 h-2 rounded-full ${
        camera.status === 'online' ? 'status-online' :
        camera.status === 'error' ? 'status-alert' : 'status-offline'
      }`} />
    </div>
  );
}
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-700">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">{camera.zone} • {camera.floor}</p>
            </div>
          </div>
          <button
            onClick={onClose}
