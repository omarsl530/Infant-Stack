...
const ConfigEditor: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Failed to save configuration. Key might already exist or value format is invalid.');
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="glass-card overflow-hidden">
        <table className="admin-table">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={6} className="text-center py-8 text-slate-400">Loading configurations...</td></tr>
            ) : filteredConfigs.length === 0 ? (
               <tr><td colSpan={6} className="text-center py-8 text-slate-400">No settings found.</td></tr>
            ) : (
                filteredConfigs.map((config) => (
                    <tr key={config.key}>
                    <td className="font-mono text-admin-400 font-medium">{config.key}</td>
                    <td className="max-w-xs truncate" title={String(config.value)}>
                        {String(config.value)}
                    </td>
                    <td><span className="badge bg-slate-700 text-slate-300">{config.type}</span></td>
                    <td className="text-slate-400">{config.description || '-'}</td>
                    <td>
                        {config.is_public ? (
                            <span className="text-xs bg-emerald-900/30 text-emerald-400 px-2 py-0.5 rounded border border-emerald-800">Public</span>
                        ) : (
                            <span className="text-xs bg-slate-700/50 text-slate-400 px-2 py-0.5 rounded border border-slate-600">Internal</span>
                        )}
                    </td>
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                required
                            />
                            {formData.type === 'boolean' && <p className="text-xs text-slate-500 mt-1">Use "true" or "false"</p>}
                        </div>
                        ...
                        {!editingConfig && (
                             <div className="flex items-center gap-2">
                                ...
                                />
                                <label htmlFor="is_public" className="text-sm text-slate-300">Public (Exposed to unauthenticated clients)</label>
                             </div>
                        )}
