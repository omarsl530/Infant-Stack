...
const AuditLogViewer: React.FC = () => {
  ...
  const [options, setOptions] = useState<{ actions: string[], resource_types: string[] }>({
    actions: [],
    resource_types: [],
  });
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-4 text-center text-sm text-gray-500">No audit logs found matching criteria.</td>
                </tr>
              ) : (
                logs.map((log) => (
                  <tr key={log.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                        ${log.action.includes('delete') ? 'bg-red-100 text-red-800' : 
                          log.action.includes('create') || log.action.includes('post') ? 'bg-green-100 text-green-800' : 
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <div className="flex flex-col">
                        ...
                        {log.resource_id && <span className="text-xs text-gray-400">{log.resource_id}</span>}
                      </div>
                    </td>
          ...
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">
                Showing page <span className="font-medium">{page}</span> of <span className="font-medium">{Math.ceil(total / 20)}</span>
                <span className="ml-2 text-gray-400">({total} total results)</span>
              </p>
            </div>
            <div>
