...
function InfantCard({ infant, onViewDetails }: { infant: Infant; onViewDetails: () => void }) {
  return (
    <div className={`glass-card p-5 transition-all hover:border-blue-500/50 hover:shadow-lg hover:shadow-blue-500/10 ${
      ...
      <div className="space-y-2 text-sm">
        <div className="flex justify-between">
          ...
          <span>{infant.ward} - Room {infant.room || 'N/A'}</span>
        </div>
        <div className="flex justify-between">
...
function AlertItem({ alert, onDismiss }: { alert: Alert; onDismiss: () => void }) {
  ...
  return (
    <div className={`p-4 border-l-4 ${severityStyles[alert.severity]} rounded-r-lg mb-3 group`}>
...
export default function App() {
  ...
  // Form states
  const [motherForm, setMotherForm] = useState({ name: '', room: '', contact: '' });
  const [infantForm, setInfantForm] = useState({ name: '', ward: '', room: '', dob: '', weight: '', motherId: '' });
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setInfants(infantsData.map(i => ({
        ...
        status: (i.tag_status === 'active' ? 'active' : i.tag_status === 'alert' ? 'alert' : 'inactive') as Infant['status'],
        lastSeen: 'Just now',
        dateOfBirth: i.date_of_birth || undefined,
      })));

      setMothers(mothersData.map(m => ({
        id: m.id,
        tagId: m.tag_id,
        name: m.name,
        room: m.room || '',
  ...
  const handleAddInfant = async () => {
    ...
    try {
      ...
      setInfantForm({ name: '', ward: '', room: '', dob: '', weight: '', motherId: '' });
  ...
  return (
    <div className="min-h-screen bg-slate-900">
      ...
      {/* Add Mother Modal */}
      <Modal isOpen={showAddMother} onClose={() => setShowAddMother(false)} title="Add New Mother">
        <p className="text-sm text-slate-400 mb-4">Tag ID: <span className="text-blue-400 font-mono">{nextMotherTag}</span></p>
        <FormInput 
          label="Full Name" 
      ...
      {/* Add Infant Modal */}
      <Modal isOpen={showAddInfant} onClose={() => setShowAddInfant(false)} title="Add New Baby">
        <p className="text-sm text-slate-400 mb-4">Tag ID: <span className="text-blue-400 font-mono">{nextInfantTag}</span></p>
        <FormInput 
          label="Baby Name" 
        ...
        <FormSelect
          ...
          options={mothers.map(m => ({ value: m.id, label: `${m.name} (${m.tagId})` }))}
        />
        <p className="text-xs text-slate-500 -mt-2 mb-4">Leave empty to assign later</p>
        <button 
      ...
      <Modal 
        ...
        onClose={() => { setSelectedInfant(null); setPairingMotherId(''); }} 
          ...
          <div className="space-y-4">
            ...
            <div className="bg-slate-900/50 rounded-lg p-4 space-y-3">
              <div className="flex justify-between">
                ...
                <span className="font-mono text-blue-400">{selectedInfant.tagId}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                <span className="text-slate-400">Mother</span>
                <span className={selectedInfant.motherName !== 'Unassigned' ? 'text-pink-400 font-medium' : 'text-slate-500'}>
                  {selectedInfant.motherName}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Mother Tag</span>
                <span className="font-mono text-pink-400">{selectedInfant.motherTagId || 'Not assigned'}</span>
              </div>
              {selectedInfant.dateOfBirth && (
                <div className="flex justify-between">
                  <span className="text-slate-400">Date of Birth</span>
                  <span>{new Date(selectedInfant.dateOfBirth).toLocaleDateString()}</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-slate-400">Last Seen</span>
                <span className="text-emerald-400">{selectedInfant.lastSeen}</span>
              </div>
            </div>
            ...
            {!selectedInfant.motherTagId && (
              <div className="mt-4 pt-4 border-t border-slate-700">
                <h4 className="font-medium mb-3 text-sm text-slate-300">Pair with Mother</h4>
                <div className="gap-2">
                  <FormSelect
                    label="Select Mother"
                    ...
                    options={[
                      ...
                      ...mothers.map(m => ({ value: m.id, label: `${m.name} (${m.tagId})` }))
                    ]}
                  />
                  <button 
      ...
      <Modal isOpen={showSettings} onClose={() => setShowSettings(false)} title="Settings">
        <div className="space-y-4">
          ...
          <div className="bg-slate-900/50 rounded-lg p-4">
            ...
            {mothers.length === 0 ? (
              <p className="text-sm text-slate-400">No mothers registered yet.</p>
            ) : (
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {mothers.map((mother) => (
                  <div key={mother.id} className="flex items-center justify-between bg-slate-800/50 rounded-lg p-2">
                    <div>
